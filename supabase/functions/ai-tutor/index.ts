...
const MODE_PROMPTS: Record<string, string> = {
    chat: "",
    study_buddy:
        "Tone: warm, encouraging study friend. Use short paragraphs, friendly emojis sparingly, ask one check-in question at the end.",
    strict_teacher:
        "Tone: rigorous teacher. Demand precision. Point out common mistakes. End with one challenge problem.",
    exam: "Mode: exam-style. Give exam-ready answers: definition → derivation → worked example → 2 practice questions.",
    simple_bangla:
        "Mode: explain in simple, everyday Bangla (Banglish ok). Use everyday analogies. Keep English only for technical terms.",
    deep_explain:
        "Mode: deep explanation. Start with intuition, then formal definition, then 2 worked examples (one easy, one harder), then edge cases.",
    fast_revision:
        "Mode: fast revision. ONE-PAGE summary only — key formulas, 3 bullet points per concept, no fluff.",
    coding_mentor:
        "Mode: coding mentor. Always include a runnable code snippet. Explain time/space complexity. Suggest one refactor.",
    summary:
        "Task: Produce a 5-7 bullet point summary of the key concepts likely covered in this lesson based on the title and any provided transcript.",
        ...
        if (attachments.length > 0) {
            ...
            for (const att of attachments as Array<{ path: string; mime: string; name?: string }>) {
                ...
                if (dErr || !blob) {
                    ...
                    return json(
                        { error: `Could not read attachment: ${att.name ?? att.path}` },
                        400,
                    );
                }
                ...
                if (mime === "application/pdf") {
                    try {
                        const { extractText, getDocumentProxy } =
                            await import("https://esm.sh/unpdf@0.12.1");
                        const doc = await getDocumentProxy(buf);
                        const { text } = await extractText(doc, { mergePages: true });
                        const cleaned = (Array.isArray(text) ? text.join("\n") : text).trim();
                        const snippet =
                            cleaned.length > 14000
                    ...
                    imageParts.push({
                        type: "image_url",
                        image_url: { url: `data:${mime};base64,${btoa(bin)}` },
                    });
                } else {
        ...
        if (module_id) {
            ...
            if (mod) {
                ...
                if (tr?.status === "ready" && tr.text) {
                    const snippet =
                        tr.text.length > 12000
                            ? tr.text.slice(0, 12000) + "\n…[truncated]"
                            : tr.text;
