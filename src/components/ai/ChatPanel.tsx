...
export const ChatPanel = ({
    ...
    useEffect(() => {
        try {
            localStorage.setItem(STORAGE_KEY(userId, sourceKey), JSON.stringify(messages));
        } catch {
            /* noop */
        }
    ...
    const send = useCallback(
        async (rawText?: string) => {
            ...
            setInput("");
            const attachmentSnapshot = attachments.items.map((a) => ({
                path: a.path,
                mime: a.mime,
                name: a.name,
            }));
                ...
                if (usageHdr) {
                    try {
                        ...
                        if (parsed && typeof parsed.count === "number")
                            onUsageUpdate(parsed as UsageState);
                    } catch {
                        /* noop */
                    }
                }
                ...
                if (res.status === 429) {
                    ...
                    if (body?.error === "limit_reached") {
                        toast.error("Daily limit reached", {
                            description: body.message ?? "Upgrade for unlimited AI.",
                        });
                    } else {
                    ...
                    {messages.length === 0 ? (
                        ...
                        <div className="space-y-6">
                            {messages.map((m, i) => (
                                ...
                                >
                                    {m.role === "assistant" && (
                                        <div className="h-7 w-7 shrink-0 rounded-lg bg-primary/15 text-primary grid place-items-center text-xs font-mono font-semibold mt-0.5">
                                            V
                                        </div>
                                    )}
                                    <div
                ...
                <div className="max-w-3xl mx-auto">
                    {attachments.items.length > 0 && (
                        <div className="mb-2 flex flex-wrap gap-2">
                            {attachments.items.map((a) => (
                                <div
                                    key={a.id}
                                    className="group relative flex items-center gap-2 rounded-xl border border-border/60 bg-card/80 px-2 py-1.5 pr-7 text-xs"
                                >
                                    {a.previewUrl ? (
                                        <img
                                            src={a.previewUrl}
                                        ...
                                        <div className="h-8 w-8 rounded bg-rose-500/15 text-rose-500 grid place-items-center">
                    ...
                    <div className="flex items-end gap-2 rounded-2xl border border-border/60 bg-card/60 px-2 py-2 focus-within:border-primary/60 transition-colors">
                        ...
                        <Button
                            ...
                            onClick={() => {
                                if (!isPaid) {
                                    toast.error("File uploads are for paid users only.", {
                                        description: "Upgrade to attach PDFs or images.",
                                    });
                                    return;
                                }
                            ...
                            title={
                                isPaid
                                    ? "Attach PDF or image (max 10MB, 5 files)"
                                    : "Paid users only"
                            }
                        >
                        ...
                        <Textarea
                            ...
                            rows={1}
                            placeholder={
                                source ? `Ask about "${source.moduleTitle}"…` : "Ask Vert anything…"
                            }
                        ...
                        {busy ? (
                            ...
                            <Button
                                onClick={() => send()}
                                disabled={
                                    !input.trim() || attachments.items.some((a) => a.uploading)
                                }
                                size="icon"
