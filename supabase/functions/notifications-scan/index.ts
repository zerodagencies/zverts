...
const T = {
    streak_risk: (streak: number): Tpl =>
        pick([
            {
                title: "Streak fragile mode 👀",
                body: `${streak} দিনের consistency ভাঙতেছে — ৫ মিনিট দেন!`,
            },
            { title: "আজকে না পড়লে streak break 😶", body: "১টা ছোট lesson = streak safe" },
        ]),
    comeback_1d: (): Tpl => ({
        title: "কালকে থামছিলেন, আজকে আবার? 😎",
        body: "ZverTs এ ফেরত আসেন boss",
    }),
    ...
    comeback_14d: (): Tpl => ({
        title: "Comeback দিবেন নাকি? 🔥",
        body: "Skill কিন্তু বসে থাকলে বাড়ে না",
    }),
    morning_push: (): Tpl =>
        pick([
    ...
    try {
        ...
        for (const u of streakUsers ?? []) {
            ...
            const inactiveDays = last
                ? Math.floor((Date.parse(todayBD) - Date.parse(last)) / 86400000)
                : 99;
            ...
            if (last === yestBD) {
                ...
                const { data } = await supabase.rpc("dispatch_notification", {
                    _user_id: u.id,
                    _category: "streak_risk",
                    _title: t.title,
                    _body: t.body,
                    _priority: "high",
                    _deep_link: "/dashboard",
                    _payload: { streak: u.current_streak },
                    _dedupe_key: `streak_risk:${todayBD}`,
                    _cooldown_hours: 12,
                });
                if (data) dispatched++;
            }
            ...
            if (cat) {
                ...
                const { data } = await supabase.rpc("dispatch_notification", {
                    ...
                    _title: t.title,
                    _body: t.body,
                    _priority: cat === "comeback_14d" ? "high" : "normal",
                    _deep_link: "/dashboard",
                    _payload: { inactive_days: inactiveDays },
                    _dedupe_key: `${cat}:${todayBD}`,
                    _cooldown_hours: 24,
                });
                if (data) dispatched++;
            }
        ...
        for (const b of behaviorUsers ?? []) {
            ...
            if (b.last_lesson_at && Date.now() - Date.parse(b.last_lesson_at) < 6 * 3600 * 1000)
                continue;
