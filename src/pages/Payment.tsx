...
const Payment = () => {
    ...
    const submit = async () => {
        if (!sender.trim() || !trx.trim()) {
            toast.error("Fill all fields");
            return;
        }
        if (!/^\d{8,20}$/.test(sender.replace(/\s/g, ""))) {
            toast.error("Invalid sender number");
            return;
        }
        if (!agreed) {
            toast.error("Please agree to the Refund Policy to continue");
            return;
        }
        setSubmitting(true);
        ...
        if (error) {
            const msg =
                error.message.includes("duplicate") || error.message.includes("payments_trx_unique")
    ...
    return (
        <AppShell>
            <section className="container py-10 md:py-14 max-w-3xl">
                ...
                <h1 className="font-display text-4xl font-semibold tracking-tight mt-3">
                    Complete payment
                </h1>
                ...
                <div className="mt-6 rounded-2xl border border-primary/40 bg-gradient-card p-6 shadow-card flex items-center justify-between">
                    ...
                    <div className="text-right">
                        <div className="font-display text-4xl font-bold">
                            {pkg.price}{" "}
                            <span className="text-base font-normal text-muted-foreground">Tk</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-8 rounded-2xl border border-border bg-card p-6">
                    ...
                    <ol className="space-y-3 text-sm">
                        <li>
                            Open your <b>{METHOD_LABELS[method]}</b> app and choose{" "}
                            <b>Send Money</b>.
                        </li>
                        <li>
                            Send <b className="text-primary">{pkg.price} Tk</b> to this merchant
                            number:
                        </li>
                    </ol>
                    <div className="mt-3 flex items-center gap-2 rounded-lg border border-primary/30 bg-primary/5 p-3">
                        ...
                        <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => copy(merchantNumber, "num")}
                        >
                            {copied === "num" ? (
                    ...
                    <p className="mt-3 text-xs text-muted-foreground font-mono">
                        After sending, paste your sender number and transaction ID below.
                    </p>
                </div>
                ...
                <div className="mt-8 rounded-2xl border border-border bg-card p-6 space-y-4">
                    ...
                    <Button
                        ...
                        className="w-full bg-gradient-lime text-primary-foreground shadow-glow disabled:opacity-50"
                    >
                        {submitting ? (
                            <>
                    ...
                    <p className="text-xs text-muted-foreground text-center font-mono">
                        ...
                        <Link to="/refund-policy" className="hover:text-primary underline">
                            Refund Policy
                        </Link>
                    </p>
                </div>
