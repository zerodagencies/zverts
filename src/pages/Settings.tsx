...
const Settings = () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            const [{ data: prof }, { data: prefs }, { data: prog }] = await Promise.all([
                supabase
                    .from("profiles")
                    .select("name,email,avatar_url,certificate_name,preferred_language")
                    .eq("id", user.id)
                    .maybeSingle(),
                supabase
                    .from("user_preferences")
                    .select(
                        "daily_goal_minutes,study_reminders_enabled,notify_email,notify_inactivity,notify_completion,profile_public",
                    )
                    .eq("user_id", user.id)
                    .maybeSingle(),
                supabase
                    .from("user_progress")
                    .select("total_gems,total_xp,current_streak,longest_streak")
                    .eq("user_id", user.id)
                    .maybeSingle(),
            ]);
            if (prof)
    ...
    if (!p)
        return (
            <AppShell>
                <div className="container py-20 text-muted-foreground font-mono text-sm">
                    Loading…
                </div>
            </AppShell>
        );
                        ...
                        <div className="rounded-2xl border border-border bg-gradient-card p-8 shadow-card space-y-6">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground mt-1">
                                    Your account is managed by Google. To change your password or
                                    recovery options, visit your Google account settings.
                                </p>
                            </div>
                            <div className="border-t border-border pt-6 flex items-center justify-between gap-4 flex-wrap">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Ends every active session on every device.
                                    </p>
                                </div>
                        ...
                        <div className="rounded-2xl border border-border bg-gradient-card p-8 shadow-card space-y-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Show your name and avatar on the leaderboard and shared
                                        courses.
                                    </p>
                                </div>
                                <Switch
                                    checked={p.profile_public}
                                    onCheckedChange={(v) => update({ profile_public: v })}
                                />
                            </div>
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    The course and all its modules and progress will be permanently
                                    removed.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
