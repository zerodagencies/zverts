...
export const DailyChallenge = ({ userId }: { userId: string }) => {
    ...
    const [done, setDone] = useState<{ score: number; total: number; passed: boolean } | null>(
        null,
    );
        ...
        <div className="rounded-2xl border border-border bg-gradient-card p-6 shadow-card">
            ...
            {done ? (
                <div className="text-center py-6">
                    <div
                        className={cn(
                            "h-14 w-14 mx-auto rounded-full flex items-center justify-center mb-3",
                            done.passed
                    ...
                    <div className="font-display text-xl">
                        {done.passed ? "Passed!" : "Try tomorrow"}
                    </div>
                    <div className="text-sm text-muted-foreground mt-1">
                        Scored {done.score} / {done.total} today
                    </div>
                </div>
            ) : questions.length === 0 ? (
                <p className="text-sm text-muted-foreground py-6 text-center font-mono">
                    Complete a module with quizzes to unlock today's challenge.
                </p>
            ) : (
                                ...
                                {q.options.map((opt: string, idx: number) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {opt}
                                            {correct && (
                                                <Check className="inline h-3.5 w-3.5 ml-2" />
                                            )}
                                        </button>
                                    );
                                })}
